...
function billOut(){
	...
	carts.forEach((cart,i)=>{
		if(i < (carts.length -1)){
			str += cart.item_id + ":"+cart.count+","
		}else{
			str += cart.item_id + ":"+cart.count
		}
	
	});
	 fetch("http://localhost:8080/MyShop/ApiController?route=bill_out&cart_data="+str)
	 .then((response)=>{
		 return response.text();
	 })
	 .then((res)=>{
...
function loadData(){
	let str = "";
	 ...
	 products.forEach((product)=>{
		 ...
		 `;
		 total += product.price * getSingleItemCount(product.id);
});
let tax = ((total / 100) * 5).toFixed(2);
let gTotal = Number(total) + Number(tax);
document.querySelector("#tbody").innerHTML = str;
document.querySelector("#gTotal").innerHTML = "$ " + total ;
document.querySelector("#gTax").innerHTML = "$ " + tax; 
document.querySelector("#ggTotal").innerHTML = "$ " + gTotal;
}
